...
field — body shape alone cannot distinguish them. We split off the path
explicitly with `urlsplit` so a query string (Starlette's `str(request.url)`
includes one) cannot defeat the suffix match.
